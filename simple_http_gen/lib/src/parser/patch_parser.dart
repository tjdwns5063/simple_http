...
final patchTypeChecker = const TypeChecker.fromRuntime(PATCH);

class PatchParser extends MethodParser
    with PathParser, QueryParser, UriParser, HeaderParser, BodyParser {
  @override
  String parse(Element method, String baseUrl) {
    return _parsePatch(baseUrl, method as MethodElement);
  }

  String _parsePatch(String baseUrl, MethodElement element) {
    return '''
    String endPoint = '${parsePath(element, patchTypeChecker)}';
    ...
    http.Response response = await http.patch(uri, headers: headerParam, body: body);
